...
export async function authorizeAgentAccess(request: NextRequest, requireWrite: boolean) {
   ...
   if (organizationError) {
      return {
         ok: false as const,
         response: NextResponse.json({ error: 'Unable to authorize Agent access.' }, { status: 500 }),
      };
   }
   if (!organization) {
      return {
