...
export async function authorizeWorkspaceMemberAccess(
   request: NextRequest,
   ...
   if (!organization) return { response: NextResponse.json({ error: 'Not found.' }, { status: 404 }) } as const;
   ...
   if (!membership) return { response: NextResponse.json({ error: 'Forbidden.' }, { status: 403 }) } as const;
