...
export default function IssueDetails() {
   ...
   const demoDetail = useMemo(
      () => (!workspace.configured && issue ? getIssueDetail(issue) : null),
      [issue, workspace.configured]
   );
   ...
   const subIssues = (demoDetail?.subIssueIds ?? [])
         ...
         <div className="flex-1 min-w-0 h-full overflow-y-auto">
            <div className="max-w-3xl mx-auto px-8 py-10">
               ...
               {workspace.configured ? (
                  <PersistentIssueDescription issue={issue} />
               ) : (
                  <div className="mt-6">
                     <ContentBlocks blocks={demoDetail!.description} />
                  </div>
               )}
                  ...
                  <div className="mt-8">
                     {subIssues.length > 0 ? (
                        <>
                           <h2 className="text-sm font-medium mb-1">
                              ...
                              <span className="text-muted-foreground">
                                 {subIssues.filter((subIssue) => subIssue.status.category === 'completed').length}/
                                 {subIssues.length}
                              </span>
                           </h2>
                        ...
                        <button className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
                           <Plus className="size-4" />Add sub-issues
                        </button>
                     )}
                  </div>
               )}
               <div className="border-t border-border/60 mt-8" />
               <ActivityFeed activity={demoDetail?.activity ?? []} issueId={issue.id} />
            </div>
         </div>
         <aside className="hidden lg:block w-80 shrink-0 border-l h-full overflow-y-auto bg-container px-5 py-6">
            <IssuePropertiesPanel issue={issue} detail={demoDetail} />
         </aside>
      </div>
   );
}
