            ...
            <div className="mx-auto max-w-3xl px-6 py-8 lg:px-10">
               {canWrite ? (
                  <form onSubmit={submit} className="rounded-lg border bg-container p-4">
                        ...
                        {kind === 'update' && (
                           ...
                           >
                              {(Object.keys(HEALTH_LABEL) as ProjectUpdateHealth[]).map((value) => (
                                 <option key={value} value={value}>{HEALTH_LABEL[value]}</option>
                              ))}
                           </select>
                        )}
                     </div>
                     ...
                     <div className="mt-3 flex items-center justify-between gap-3">
                        <p className="text-xs text-muted-foreground">
                           Agent drafting and file attachments are not connected yet, so those controls are intentionally hidden.
                        </p>
                  ...
                  <div className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
                     Guest access is read-only. Existing project updates remain visible below.
                  </div>
               )}
...
export default function ProjectActivityRuntime({ projectId }: { projectId: string }) {
   const workspace = useWorkspace();
   return workspace.configured ? <PersistentProjectActivity projectId={projectId} /> : <ProjectActivity projectId={projectId} />;
}
