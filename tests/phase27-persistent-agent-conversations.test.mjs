...
test('Agent API authenticates membership, rejects unsafe mutations and persists server-derived replies', async () => {
   ...
   assert.match(route, /parseAgentPostBody/);
   assert.match(route, /chatTitleFrom\(body\.input\)/);
   assert.match(route, /getAgentReply\(body\.input\)/);
...
test('configured Agent UI hydrates and queues persistence while retaining the demo stream', async () => {
   ...
   assert.match(component, /persistenceError/);
   ...
   assert.match(store, /appendToMessage/);
   assert.match(store, /finishMessage/);
});
